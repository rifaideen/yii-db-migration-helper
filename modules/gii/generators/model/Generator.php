...
class Generator extends \yii\gii\Generator
{
    ...
    public $nonFilters = 'limit, offset, orderBy, order';
    ...
    {
        return 'This generator generates an JS model for the specified database table.';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['db', 'ns', 'tableName', 'modelClass', 'baseClass', 'queryNs', 'queryClass', 'queryBaseClass', 'privateFields', 'nonFilters'], 'filter', 'filter' => 'trim'],
    ...
    public function hints()
    {
        return array_merge(parent::hints(), [
            ...
            'privateFields' => 'Fields that should not be exposed to API. CSV supported',
            'nonFilters' => 'Fields that are not considerable by API to filter results. CSV supported'
        ]);
    }
    ...
    {
        return array_merge(parent::stickyAttributes(), ['ns', 'db', 'baseClass', 'generateRelations', 'generateLabelsFromComments', 'queryNs', 'queryBaseClass', 'useTablePrefix', 'generateQuery', 'generatorPath']);
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $this->columns = implode(',', array_keys($this->generateProperties($tableSchema)));
            $params = [
                'tableName' => $tableName,
                ...
                'rules' => $this->generateJSRules($tableSchema),
                'relations' => isset($relations[$tableName]) ? $relations[$tableName] : [],
                'nonFilters' => array_filter(explode(',', $this->nonFilters)),
                'privateFields' => array_filter(explode(',', $this->privateFields)),
                'columns' => $this->columns
            ];
            $files[] = new CodeFile(
                $this->generatorPath . str_replace('\\', '/', $this->ns) . '/' . strtolower($modelClassName) . '.js',
                $this->render('model-js.php', $params)
            );
     ...
     * @return array the generated validation rules
     */
    public function generateJSRules($table)
    {
        $types = [];
        $lengths = [];
        foreach ($table->columns as $column) {
            ...
            switch($type) {
                case 'number':
                    foreach ($columns as $column) {
                        ...
                        $rules[$column] = "Joi.number().positive().description('$columnName')";
                    }
                ...
                case 'integer':
                    foreach ($columns as $column) {
                        $columnName = ucwords(str_replace('_', ' ', $column));
                        $rules[$column] = "Joi.number().integer().positive().description('$columnName')";
                    }
                ...
                case 'safe':
                    foreach ($columns as $column) {
                        $columnName = ucwords(str_replace('_', ' ', $column));
                        $rules[$column] = "Joi.string().trim().min(3).max(255).description('$columnName')";
                    }
                ...
                case 'boolean':
                    foreach ($columns as $column) {
                        $columnName = ucwords(str_replace('_', ' ', $column));
                        $rules[$column] = "Joi.boolean().default(false).description('$columnName')";
                    }
        ...
        foreach ($lengths as $length => $columns) {
            foreach ($columns as $column) {
                $columnName = ucwords(str_replace('_', ' ', $column));
                $rules[$column] = "Joi.string().trim().max($length).description('$columnName')";
            }
        }
        ...
        foreach ($schemaNames as $schemaName) {
            foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $refs) {
                    ...
                    $link = $this->generateJSRelationLink(array_flip($refs));
                    $relationName = $this->generateRelationName($relations, $table, $fks[0], false);
                    $relations[$table->fullName][$relationName] = [
                        "Api.BelongsToOneRelation",
                        $refClassName,
                        false,
                        $link
                    ];
                    ...
                    $link = $this->generateJSRelationLink($refs);
                    $relationName = $this->generateRelationName($relations, $refTableSchema, $className, $hasMany);
                    $relations[$refTableSchema->fullName][$relationName] = [
                        $hasMany ? 'Api.HasManyRelation' : 'Api.BelongsToOneRelation',
                        $className,
                        $hasMany,
                        $link
                    ];
    ...
    protected function generateJSRelationLink($refs)
    {
        $pairs = [];
        foreach ($refs as $a => $b) {
            $pairs[] = ['from' => $a, 'to' => $b];
